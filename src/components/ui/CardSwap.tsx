...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  const handleCardClick = (clickedIdx: number) => {
    ...
    const pos = currentOrder.indexOf(clickedIdx);
    if (pos <= 0) return;

    const newOrder = [...currentOrder.slice(pos), ...currentOrder.slice(0, pos)];
  ...
  useEffect(() => {
    const handleBringFront = (e: Event) => {
      const customEvent = e as CustomEvent<number>;
      if (typeof customEvent.detail === 'number') {
        handleCardClick(customEvent.detail);
      }
    };
    window.addEventListener('bring-card-front', handleBringFront);
    return () => {
      window.removeEventListener('bring-card-front', handleBringFront);
    };
