        ...
        <div className="flex flex-col lg:flex-row items-center justify-between relative gap-4 lg:gap-0">
              ...
              <a
                ...
                onClick={(e) => {
                  if (item.href.startsWith('#')) {
                    ...
                    const el = document.getElementById(targetId) || document.getElementById('skills');
                    if (el) {
                      el.scrollIntoView({ behavior: 'smooth' });
                    }
                    if (item.href === '#projects' || item.label.toLowerCase().includes('project')) {
                      window.dispatchEvent(new CustomEvent('bring-card-front', { detail: 1 }));
                    } else if (item.href === '#skills' || item.label.toLowerCase().includes('skill')) {
                      window.dispatchEvent(new CustomEvent('bring-card-front', { detail: 0 }));
                    }
                  }
                }}
                ...
                className="relative px-6 py-2.5 rounded-full bg-white/40 hover:bg-white/60 border border-white/60 text-black font-extrabold text-sm tracking-wide transition-all duration-300 overflow-hidden shadow-[0_8px_32px_0_rgba(0,0,0,0.12),inset_0_1px_2px_rgba(255,255,255,0.8)] hover:shadow-[0_12px_36px_0_rgba(0,0,0,0.18),inset_0_1px_3px_rgba(255,255,255,0.9)] hover:scale-105 active:scale-95"
              >
                {item.label}
                {hoveredIndex === idx && (
                  <motion.div
                    layoutId="navbar-indicator-left"
                    className="absolute inset-0 border-b-[3px] border-black/80"
          ...
          <div className="hidden lg:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 items-center justify-center pointer-events-auto cursor-pointer group z-10">
            <div className="relative flex items-center justify-center h-12 w-16">
              ...
              <div className="z-10 w-10 h-10 bg-white/20 border-2 border-white/40 backdrop-blur-xl rounded-[12px] flex items-center justify-center shadow-[0_8px_32px_rgba(0,0,0,0.2)] transition-all duration-300 group-hover:bg-white/30 group-hover:shadow-[0_8px_32px_rgba(255,255,255,0.25)]">
          ...
          <div className="flex flex-wrap justify-center lg:justify-end items-center gap-2 sm:gap-3 w-full lg:w-auto mt-4 lg:mt-0 z-20">
            <a
              ...
              style={{ color: "#000000" }}
              className="text-xs sm:text-sm font-extrabold tracking-wide hover:opacity-75 transition-opacity px-2"
            >
              {contactText}
            </a>
            <button 
              ...
              className="px-6 py-2.5 rounded-full bg-[#00ffc6]/80 hover:bg-[#00ffc6] border border-white/70 text-black text-xs sm:text-sm font-black tracking-wide transition-all duration-300 flex items-center gap-2 shadow-[0_8px_32px_0_rgba(0,255,198,0.35),inset_0_1px_2px_rgba(255,255,255,0.9)] hover:shadow-[0_12px_40px_0_rgba(0,255,198,0.5),inset_0_1px_3px_rgba(255,255,255,1)] hover:scale-105 active:scale-95"
            >
              {ctaText}
            </button>
          </div>
...
export function RobotHero({
  ...
  return (
    ...
    >
      ...
      <div 
        className="absolute inset-0 z-5 pointer-events-none flex flex-col pt-24 sm:pt-28 md:pt-32 pb-16 sm:pb-20 md:pb-24 lg:pb-28"
      >
        <div className="w-full max-w-[1400px] mx-auto px-8 md:px-12 lg:px-16 flex-1 flex flex-col justify-between">
          
          <div className="text-left select-none ml-2 md:ml-8 lg:ml-12">
            <h3 className="font-sans font-bold text-[#383838] text-lg sm:text-xl md:text-2xl tracking-wider uppercase mb-1">
              Hello, I'm
            </h3>
            <h1 className="font-sans font-extrabold text-black text-3xl sm:text-4xl md:text-5xl lg:text-6xl tracking-tight leading-none whitespace-nowrap">
              Abhigyan Padhi
            </h1>
          </div>

          <div className="text-right select-none mr-2 md:mr-8 lg:mr-12">
            <h2 className="font-sans font-extrabold text-black text-2xl sm:text-3xl md:text-4xl lg:text-5xl tracking-tight whitespace-nowrap">
      ...
      <div className="absolute inset-0 z-20 pointer-events-none flex flex-col justify-between">
        ...
        <div 
          className={`pb-6 sm:pb-8 flex flex-col items-center justify-center gap-1.5 transition-all duration-500 group self-center ${
            scrolled ? 'opacity-0 pointer-events-none translate-y-4' : 'opacity-80 hover:opacity-100 pointer-events-auto cursor-pointer'
          }`}
          onClick={() => {
            window.scrollTo({ top: window.innerHeight, behavior: 'smooth' });
          }}
        >
          <span className="text-[10px] sm:text-xs font-extrabold text-black tracking-[0.25em] uppercase select-none group-hover:translate-y-0.5 transition-transform">
            Scroll to know more
          </span>
          <div className="w-5 h-8 border-2 border-black/80 rounded-full flex justify-center pt-1.5 transition-colors group-hover:border-black">
